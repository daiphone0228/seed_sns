...
if (!empty($_POST)) {
  if (isset($_POST['tweet']) && $_POST['tweet'] != '') {
    $sql = sprintf('INSERT INTO `tweets`(`tweet`, `member_id`, `reply_tweet_id`, `created`) VALUES ("%s",%d,%d,now())',
      m($db, $_POST['tweet']),
...
// HTML文に挿入する前に、最初に下記のように入力して値が出ることを確認してから挿入するとわかりやすい
...
if (isset($_GET['search_word']) && !empty($_GET['search_word'])) {
  $sql = sprintf('SELECT m.nick_name, m.picture_path, t.* FROM members m, tweets t WHERE m.member_id=t.member_id AND t.tweet LIKE "%%%s%%" ORDER BY t.modified DESC LIMIT %d, 5',
    m($db, $_GET['search_word']),
    $start);
} else {
  $sql = sprintf('SELECT m.nick_name, m.picture_path, t.* FROM members m, tweets t WHERE m.member_id=t.member_id ORDER BY t.modified DESC LIMIT %d, 5',
    $start);
}
...
<html lang="ja">
  <head>
    ...
    <title>SeedSNS - members page</title>

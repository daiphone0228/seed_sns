...
if (!empty($_POST)) {
  $sql = sprintf('INSERT INTO `members`(`nick_name`, `email`, `password`, `picture_path`, `created`) VALUES ("%s","%s","%s","%s",now())',
  m($db, $_SESSION['join']['nick_name']),
  m($db, $_SESSION['join']['email']),
  m($db, sha1($_SESSION['join']['password'])),
  m($db, $_SESSION['join']['picture_path'])
  );
...
<html lang="ja">
  <head>
    ...
    <title>SeedSNS - check your informations</title>
